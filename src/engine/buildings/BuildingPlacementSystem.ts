...
export class BuildingPlacementSystem {
  ...
  getDefId(): string { return this.defId; }
  getPreviewTx(): number { return this.previewTx; }
  getPreviewTy(): number { return this.previewTy; }
  isValid(): boolean { return this.valid; }

  updateMouse(screenX: number, screenY: number): void {
  ...
  private checkValid(): void {
    ...
    if (!def) { this.valid = false; return; }
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    const centerScreen = this.iso.worldToScreen(
      this.previewTx + def.size / 2,
      this.previewTy + def.size / 2, 0, this.camera
    );
