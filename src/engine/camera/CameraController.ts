...
export class CameraController {
  ...
  constructor(camera: Camera, eventBus: EventBus) {
    ...
    eventBus.on('input:wheel', ({ delta, screenX, screenY }) => {
      ...
      eventBus.emit('camera:moved', undefined as never);
    });
  ...
  update(dt: number): void {
    ...
    if (this.isDown('KeyW') || this.isDown('ArrowUp')) dy += speed;
    if (this.isDown('KeyS') || this.isDown('ArrowDown')) dy -= speed;
    if (this.isDown('KeyA') || this.isDown('ArrowLeft')) dx += speed;
    if (this.isDown('KeyD') || this.isDown('ArrowRight')) dx -= speed;

    // Edge scroll (only when mouse is inside the canvas)
    if (this.mouseInCanvas) {
      const { canvasWidth, canvasHeight } = this.camera;
      if (this.mouseX >= 0 && this.mouseX < EDGE_SCROLL_THRESHOLD) dx += edgeSpeed;
      if (this.mouseX > canvasWidth - EDGE_SCROLL_THRESHOLD) dx -= edgeSpeed;
      if (this.mouseY >= 0 && this.mouseY < EDGE_SCROLL_THRESHOLD) dy += edgeSpeed;
      if (this.mouseY > canvasHeight - EDGE_SCROLL_THRESHOLD) dy -= edgeSpeed;
    }
