...
import type { TileData, ResourceNode } from '../../types/map';
...
export interface MapGenOptions {
  ...
  height: number;
  seed: number;
  waterLevel: number;    // 0..1, fraction of map that is water
  forestDensity: number; // 0..1
  numGoldMines: number;
  numStoneMines: number;
  playerCount: number;
}

export const DEFAULT_GEN_OPTIONS: MapGenOptions = {
  width: 32,
  height: 32,
  seed: 12345,
  waterLevel: 0.25,
...
export class MapGenerator {
  generate(opts: MapGenOptions = DEFAULT_GEN_OPTIONS): MapData {
    ...
    const playerStarts: TileCoord[] = this.computePlayerStarts(tiles, width, height, opts.playerCount, rng);
  ...
  private generateHeightmap(width: number, height: number, rng: () => number): number[][] {
  ...
  private assignTerrain(h: number, m: number, waterLevel: number): TileData {
  ...
  private computeElevations(heightmap: number[][], waterLevel: number): number[][] {
  ...
  private placeForestClusters(
    tiles: TileData[][], width: number, height: number,
    ...
    for (let i = 0; i < numClusters; i++) {
      ...
      for (let dy = -radius; dy <= radius; dy++) {
        for (let dx = -radius; dx <= radius; dx++) {
          ...
          if (!tile.passable || tile.resourceId !== null) continue;
          if (rng() < 0.7) result.push({ tx, ty });
        }
      }
    }
    return result;
  ...
  private computePlayerStarts(
    tiles: TileData[][], width: number, height: number,
    playerCount: number, _rng: () => number
  ): TileCoord[] {
    ...
    for (let r = 0; r <= 5; r++) {
      for (let dy = -r; dy <= r; dy++) {
        for (let dx = -r; dx <= r; dx++) {
          ...
          if (tiles[ty][tx].passable) return { tx, ty };
        }
      }
    }
    return null;
