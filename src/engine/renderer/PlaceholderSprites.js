...
export class PlaceholderSprites {
     ...
     */
    static drawTerrainTile(ctx, terrain, sx, sy, zoom, elevation) {
     ...
     */
    static drawUnit(ctx, unitClass, sx, sy, zoom, direction, // 0-7
    playerId, selected) {
     ...
     */
    static drawBuilding(ctx, defId, sx, sy, zoom, progress, // 0..1
    playerId, isSelected) {
        const pc = PLAYER_COLORS[playerId % 8];
    ...
    static drawHealthBar(ctx, sx, sy, zoom, fraction, width = 28) {
        const w = width * zoom;
        const h = 3 * zoom;
        ctx.fillStyle = '#333333';
