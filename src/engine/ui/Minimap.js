...
export class Minimap {
    ...
    render(ctx, camera, units, canvasWidth, canvasHeight) {
        if (this.dirty)
            this.renderTerrain();
        ...
        ctx.lineWidth = 1;
        ctx.strokeRect(x + vpX1 * tileW, y + vpY1 * tileH, (vpX2 - vpX1) * tileW, (vpY2 - vpY1) * tileH);
        ctx.restore();
    }
    ...
    containsPoint(sx, sy, canvasWidth, canvasHeight) {
        const x = canvasWidth - MAP_SIZE - PADDING;
        const y = canvasHeight - MAP_SIZE - PANEL_H - PADDING;
        return sx >= x && sx <= x + MAP_SIZE && sy >= y && sy <= y + MAP_SIZE;
    }
    /** Convert minimap click to world tile coords */
    clickToWorld(sx, sy, canvasWidth, canvasHeight) {
        const x = canvasWidth - MAP_SIZE - PADDING;
        const y = canvasHeight - MAP_SIZE - PANEL_H - PADDING;
